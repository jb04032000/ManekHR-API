...
export class DeliveryChallan extends Document {
  ...
  @Prop({ type: String, default: 'delivery_challan' })
  voucherType: string;
  ...
  // DeliveryChallan-specific fields
  @Prop({
    type: String,
    enum: ['goods', 'job_work', 'sample', 'returnable'],
    required: true,
  })
  challanType: string;
  ...
  // e-Way Bill generated for this challan (challans move goods, so the e-Way bill is the
  // primary document here). Shape mirrors SaleInvoice.ewayBill so EwaybillService.computeEwb
  // can persist on either voucher. Populated by EwaybillService.generateForChallan.
  @Prop({
    type: {
      ewbNo: { type: String },
...
DeliveryChallanSchema.index({ workspaceId: 1, firmId: 1, voucherDate: -1 });
DeliveryChallanSchema.index({ workspaceId: 1, firmId: 1, voucherNumber: 1 });
