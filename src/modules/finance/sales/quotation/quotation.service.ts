...
export class QuotationService {
  ...
  async createDraft(wsId: string, firmId: string, dto: CreateQuotationDto, userId: string) {
    return withFinanceSpan(
      this.tracer,
      'finance.createQuotation',
      ...
      async () => {
        ...
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.created_quotation',
          properties: { workspaceId: wsId, firmId, quotationId: String(doc._id) },
        });
        return doc;
      },
    );
  ...
  async post(wsId: string, firmId: string, id: string, userId: string) {
    return withFinanceSpan(
      ...
      async () => {
        ...
        doc.voucherNumber = await this.voucherSeries.generateNextNumber(firmId, 'quotation', fy);
        doc.state = 'posted';
  ...
  async cancel(wsId: string, firmId: string, id: string, reason: string, userId: string) {
    return withFinanceSpan(
      ...
      async () => {
        ...
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.cancelled_quotation',
          properties: {
            workspaceId: wsId,
            firmId,
            quotationId: String(doc._id),
  ...
  async clone(wsId: string, firmId: string, id: string, userId: string) {
    return withFinanceSpan(
      ...
      async () => {
        ...
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.cloned_quotation',
          properties: {
            workspaceId: wsId,
            firmId,
            quotationId: String(clone._id),
  ...
  // ─── sendVoucher ──────────────────────────────────────────────────────────
  // D-27: email channel wired to MailService (NOT a stub).
      ...
      async () => {
        ...
        for (const channel of body.channels) {
          try {
            if (channel === 'email') {
              ...
              const attachments: any[] = [
                {
                  filename: `quotation-${(doc as any).voucherNumber ?? String(doc._id)}.pdf`,
                  content: pdfBuffer,
                  contentType: 'application/pdf',
                },
              ];

              const subject = `Quotation ${(doc as any).voucherNumber ?? '(draft)'} from ${(firm as any).firmName}`;
              const html = `
            <p>Dear ${(doc as any).partySnapshot?.name ?? 'Customer'},</p>
            <p>${body.message ?? `Please find attached quotation ${(doc as any).voucherNumber ?? ''}.`}</p>
          ...
          `;
              ...
              this.logger.warn(
                `SMS channel stubbed for voucher ${id} — provider integration pending`,
              );
