...
@Controller('workspaces/:wsId/finance/firms/:firmId/sales/quotations')
@UseGuards(JwtAuthGuard, RolesGuard, SubscriptionGuard)
@RequireSubscription({ module: AppModule.FINANCE, subFeature: 'sales_quotations' })
export class QuotationController {
  constructor(private readonly service: QuotationService) {}

  // D-24 endpoint set: list, findOne, create, update, post, cancel, clone, send, voidDraft
  ...
  send(
    ...
    // Non-invoice voucher send: email channel via QuotationService.sendVoucher (mirrors SaleInvoiceService pattern)
