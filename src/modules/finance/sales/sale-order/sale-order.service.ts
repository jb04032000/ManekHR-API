...
import { SaleOrder } from './sale-order.schema';
...
export class SaleOrderService {
  private readonly logger = new Logger(SaleOrderService.name);
  // Platform-bar observability: shared finance tracer (mirrors SaleInvoiceService).
  private readonly tracer = trace.getTracer('finance');

  constructor(
    @InjectModel(SaleOrder.name) private readonly model: Model<SaleOrder>,
  ...
  async findOne(wsId: string, firmId: string, id: string) {
    ...
    if (!doc) throw new NotFoundException('SaleOrder not found');
    return doc;
  }

  async createDraft(wsId: string, firmId: string, dto: CreateSaleOrderDto, userId: string) {
    return withFinanceSpan(
      this.tracer,
      'finance.createSaleOrder',
      ...
      async () => {
        ...
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.created_sale_order',
          properties: { workspaceId: wsId, firmId, saleOrderId: String(doc._id) },
        });
        return doc;
      },
    );
  ...
  async post(wsId: string, firmId: string, id: string, userId: string) {
    return withFinanceSpan(
      ...
      async () => {
        ...
        doc.voucherNumber = await this.voucherSeries.generateNextNumber(firmId, 'sale_order', fy);
        doc.state = 'posted';
        ...
        await doc.save();
        // Reserve stock for SO line items (D-13)
        await this.inventory.reserve(wsId, firmId, doc.lineItems);
        // Fire-and-forget product analytics on the successful post (ids / voucher no only).
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.posted_sale_order',
          properties: {
            workspaceId: wsId,
            firmId,
            saleOrderId: String(doc._id),
  ...
  async cancel(wsId: string, firmId: string, id: string, reason: string, userId: string) {
    return withFinanceSpan(
      ...
      async () => {
        ...
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.cancelled_sale_order',
          properties: {
            workspaceId: wsId,
            firmId,
            saleOrderId: String(doc._id),
  ...
  async clone(wsId: string, firmId: string, id: string, userId: string) {
    return withFinanceSpan(
      ...
      async () => {
        ...
        this.postHog.capture({
          distinctId: userId,
          event: 'sales.cloned_sale_order',
          properties: {
            workspaceId: wsId,
            firmId,
            saleOrderId: String(clone._id),
          ...
          try {
            if (channel === 'email') {
              ...
              const attachments: any[] = [
                {
                  filename: `sale-order-${(doc as any).voucherNumber ?? String(doc._id)}.pdf`,
                  content: pdfBuffer,
                  contentType: 'application/pdf',
                },
              ];
              const subject = `Sale Order ${(doc as any).voucherNumber ?? '(draft)'} from ${(firm as any).firmName}`;
              const html = `<p>Dear ${(doc as any).partySnapshot?.name ?? 'Customer'},</p><p>${body.message ?? `Please find attached sale order ${(doc as any).voucherNumber ?? ''}.`}</p><p>Thank you,<br/>${(firm as any).firmName}</p>`;
